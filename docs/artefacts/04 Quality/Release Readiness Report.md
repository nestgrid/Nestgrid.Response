...
| Q-007 / SEC-006 | P1 | Pre-remediation package graphs contained Critical/High advisories. Candidate A evidence now records patched graphs, current MVC package metadata, package hash and supported consumer execution. | Awaiting final Security closure and protected-CI provenance; no risk acceptance recorded |
...
- Candidate A Engineering evidence reports no vulnerable packages in evaluated graphs, resolving to `System.Text.Encodings.Web 4.7.2`, `Microsoft.AspNetCore.Http 2.1.22` and `Newtonsoft.Json 13.0.1`.
