...
| SEC-006 | P1 | The pre-remediation Quality report identified Critical/High advisories. Candidate A is implemented and the evaluated graphs report no vulnerable packages, but final MVC package metadata and supported MVC consumer evidence remain outstanding. | A package-closure or consumer discrepancy could leave a known vulnerable path in the release artefact. | Reconcile the current Quality position and retain authoritative MVC closure/consumer evidence before final Security closure. No exception is implied. |
